...
export function parseCliRequest(argv) {
  ...
  if (!SUPPORTED_LANGUAGES.has(language)) {
    return {
      ok: false,
      message: "Usage: node scripts/run-cognitive-cli.mjs <typescript|java> -- <tool args...>"
    };
  }
...
export async function resolveTool(language, deps = createDefaultDeps()) {
  ...
  try {
    ...
    if (cachedTool !== undefined) {
      deps.warn(`Using cached cognitive ${language} CLI ${cachedTool.version}: ${error.message}`);
      return {
        ...cachedTool,
        stale: true
      };
    ...
    throw new Error(
      `Cannot resolve cognitive ${language} CLI and no cached tool is available: ${error.message}`
    );
  }
}
